 ...
 *
 */
#define NXE2000_BATTERY_VERSION "NXE2000_BATTERY_VERSION: 2013.12.26 V3.0.0.0"
...
static void nxe2000_displayed_work(struct work_struct *work)
{
	...
	if ((NXE2000_SOCA_DISP == info->soca->status)
		...
		if(g_full_flag == 1){	/* for issue 1 solution start*/
			...
			if ((calculated_ocv > get_OCV_voltage(info, 9))
...
/* ======================================================= */
    ...
    otg_dev_mode = gpio_get_value(CFG_GPIO_OTG_USBID_DET);
    pmic_vbus    = gpio_get_value(CFG_GPIO_PMIC_VUSB_DET);
...
//			otg_phy_off();
		...
		queue_delayed_work(info->monitor_wqueue,
					&info->get_charger_work, msecs_to_jiffies(500));
    }
