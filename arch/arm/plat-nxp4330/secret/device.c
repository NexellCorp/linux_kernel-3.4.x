...
static int _dwmci2_init(u32 slot_id, irq_handler_t handler, void *data)
{
    ...
    nxp_soc_gpio_set_out_value(io, 0);
    ret = request_irq(irq, handler, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, DEV_NAME_SDHC "2", (void *)host->slot[slot_id]);
    if (0 > ret)
...
static int _wifi_power(int on)
{
    printk("%s %d\n", __func__, on);
    if (on) {
        nxp_soc_gpio_set_out_value(PAD_GPIO_D + 29, on);
        nxp_soc_gpio_set_out_value(PAD_GPIO_E + 21, on);
    }
    return 0;
}
